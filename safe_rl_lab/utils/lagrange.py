...
class PIDLagrange:
    ...
    def update(self, ep_cost_avg: float) -> None:
        ...
        #if we linearly interpolate between the adv and cost_adv
        if self._diff_norm:
            self._pid_i = max(0.0, min(1.0, self._pid_i))

        # exponential smoothing of proportional error
        a_p = self._pid_delta_p_ema_alpha
        self._delta_p *= a_p
        self._delta_p += (1 - a_p) * delta

        #exponential smoothing of derivative term
        ...
        #again normalized to [0, 1] if necessary
        if self._diff_norm:
